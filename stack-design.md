...
| 63 | Which window space `SCREEN` and the pointer name | webgl.tree docs (2026-09-15) | the DOM's: top-left, y down, viewport `[0, h, w, −h]`; host's pointer and labels in CSS pixels, webgl.tree's HUD and `space` functions in drawing-buffer pixels | GL's window coordinates — bottom-left, y up, `[0, 0, w, h]`, device pixels — what the pipeline chapter's `W` and the notation's `pointer` already describe, and `gl_FragCoord` counts | **pending, to discuss first** — proposed: a per-host `createHost(canvas, { screenYUp })`, the pointer flipped and scaled once at the DOM boundary; webgl.tree (and webgpu.tree) opt in, so its HUD, `space` functions, `pick` and labels read y-up device pixels; p5.tree keeps y-down with p5's mouse; tree unchanged (its signed viewport covers both). Folds in §2.3's "pointer in canvas pixels". Pierre wants the consequences walked through before any change |
