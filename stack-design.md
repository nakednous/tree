...
| 58 | webgl.tree's script-tag build | #56 (2026-09-14) | UMD `dist/webgl.tree.umd.cjs` (script tag, `require`, AMD), `.cjs` because the package is `"type": "module"` | as p5.tree: ESM plus IIFE `dist/webgl.tree.js`, no `require` path | **pending** — recommended: p5.tree's pair; nothing `require`s a browser GPU library, and a `.js` file serves as a script on every CDN |
