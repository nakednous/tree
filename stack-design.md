...
| 27 | `mat3Direction`'s convention | query.js (surfaced auditing p5.tree's matrix examples, 2026-09-12) | `out = to₃ · inv(from₃)`: the world direction whose coordinates in `to` equal a world direction's coordinates in `from` | `inv(to₃) · from₃`, the coordinate conversion `from → to` that `mat4Location` and `mapDirection(MATRIX → MATRIX)` perform | **pending** — the code, README and the p5 example all state and use the first; it is the odd one out of the three frame transforms, so a caller reaching for "this direction in frame B's coordinates" gets the inverse relation |
