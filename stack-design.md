...
| the morph and skinning heroes (both on three.js) | one glTF model, RobotExpressive: morph targets blended in the vertex stage by a weights uniform (morph), and a skinned skeleton with clips (skinning); the rest is furniture the modules cover | host media: a glTF loader beside `loadModel` (positions, normals, texcoords, indices, each morph target's deltas, and the skin's joints, weights, inverse bind matrices and animation clips, from the JSON plus the binary buffer) into the arrays shape; tree: the clip sampling and the joint palette (numeric, out-first); the morph and skinning vertex stages stay the notebook's, paired with `lit_phong.frag` like displacement's; webgl.tree draws the buffers as any other. The two-bone tube (`skinning.js`) is that chapter's companion, ported | — |
...
| 36 | Inertia on the orbit | host §12 (surfaced by the e13 phone and desktop runs, 2026-09-12) | none — direct manipulation wants exactness | an opt-in `inertia`: a decayed replay of the gesture's last step for orbit, pan and dolly alike, cancelled by the next touch | **damping during the drag, three.js's shape made time-based, orbit only** (Pierre, 2026-09-16; deferred 2026-09-12 with e13's per-frame coast as the departure point) — `damping` in seconds, the lag's time constant `τ`, default 0.2 (Pierre's pick after trying), 0 exact. Each step of the gesture — `[dAz, dEl, panX, panY, log dolly]` — and each wheel notch is an impulse `Δ / τ` on a rate the camera drains every frame by the exact integral `v · τ · (1 − e^(−dt/τ))`, `v` decayed by `e^(−dt/τ)`: the camera trails the finger about `τ` behind, and after a release travels the rest of what the finger did and no more, a flick easing out; the total is always the gesture's own. three.js's `enableDamping` applies `dampingFactor` of the pending delta per frame (0.05 at 60 Hz is `τ ≈ 0.33 s`); p5's `orbitControl` runs a velocity low-pass with gain, which coasts beyond the drag. A fresh touch drops what is still draining, so do `home()` and `enabled = false`. The clock is the orbit's own, `performance.now()` between updates, clamped; `update(dt)` takes the frame. tree holds the numeric piece with its golden case — `coastStep(out, rate, dt, tau)`, the step written and the rate decayed in place, `coastAlive(rate, eps)` — host's orbit the impulses, the drain and the application through the camera ops it already uses; p5.tree reaches the orbit through host's factory. Not shared: handles position exactly, helms are rates already, tracks are playback — the wider question closes here. Tried first and dropped the same day: exact during the drag with a coast at release seeded from the *average* of the last 100 ms (built, felt wrong to Pierre — with a mouse the average is near zero at the click's release, so the flick died); then `damping` alone at three's 0.33 s (better, but laggy); then a second knob `inertia`, e13's flick from the fastest step of the last 200 ms beside the lag (built, tried, "overkill" — dropped from the API the same day). Pierre's pick: `damping: 0.2`, the example runs that. Also weighed: e13's largest-step seed and per-frame decay (frame-rate dependent), the release rate fed to a camera helm (body-relative, drifts off the gaze point) |
...
| 38 | webgl.tree docs shape | webgl.tree docs (2026-09-13) | module examples: an import map sends twgl.js to the CDN and webgl.tree, tree, host to site-local ES builds | UMD examples as p5.tree's: two script tags, the `webglTree` global, no imports | **script-tag examples** (Pierre, 2026-09-14) — ruled by #56 |
...
| 41 | Packaging the OBJ parser | host (2026-09-13) | — | a runtime dependency, external in the ES build | **host's own parser** (Pierre, 2026-09-15) — `host/obj`: positions, uvs, normals, fan triangulation, one vertex per distinct triple, negative indices; the output shape unchanged. webgl-obj-loader's minified build carried `eval`, a strict-CSP hazard bundled into every bridge's script-tag build; host's runtime dependencies stay tree alone |
...
| 54 | Line quality: a line shader | webgl.tree gizmos (2026-09-13: with the wide-line program fixed — it never compiled while it used the reserved `half` — W in `testing/gizmos.html` shows wider but plain quads, far from p5's strokes) | 1-px `gl.LINES`, or `width` quads with no caps, joins or antialiasing (§7 #2) | — | **no line shader; `width` counts canvas pixels, default 1** (Pierre, 2026-09-16) — once the buffer matched the display (§7 #65) the symptom was the unit: native GL lines are one device pixel, half a canvas pixel at DPR 2, while p5's strokes count canvas pixels scaled by the density. So the one draw path scales `width` (default 1) by the canvas scale: a product at or below 1 draws the native hairline, above it the existing quad expander with that product as its device-pixel width — two device pixels by default on a dense display, one on a plain one, `width: 3` three canvas pixels everywhere, as p5's `strokeWeight`. Antialiasing comes from the canvas's own multisampling (the `antialias` context attribute, on by default) or from a `createCanvas` density above the display's, which is supersampling; render targets stay single-sampled unless `samples` asks. Caps, joins, dashes and a user line program are not built: at these widths the quads' open joins are invisible, and the agnostic approach favours what GL and the display give |
...
| 56 | Who gets a live API site | webgl.tree docs (2026-09-14) | webgl.tree's site: module examples under an import map, `@nakednous/*` named on its pages | — | **the `*.tree` bridges only** (Pierre, 2026-09-14) — webgl.tree's examples load two script tags (twgl, `webgl.tree.js`) and open with `const { …, tree, host } = webglTree`, as p5.tree's do; the index page keeps one ESM demo with its import map and install lines, the only place `@nakednous/*` is named. The site shows the user surface only: bridge internals (`contextOf`, `declaredTransforms`, `uploadTransforms`, `fill`, `expand`, `rectMatrix`, `passOf`, `targetSpecs`) stay exported but hidden. `tree`, `host` and `ui` document in their READMEs, with no site and no live examples |
| 57 | Documenting the re-exported `tree` / `host` surface | webgl.tree docs (2026-09-14) | undocumented: the namespaces are re-exported without doclets | render tree's own doclets (developer prose); flat curated re-exports (two ways to call, reverses #39) | **doc-only files in webgl.tree** (Pierre, 2026-09-14) — webgl.tree writes its own doclets and examples for a curated user surface, as p5.tree does for its matrix ops: tree's matrices, projection queries, `mapLocation` / `mapDirection`, quaternions, camera state, visibility and their constants; host's `createHost`, handles, labels, orbit, devices and media. Tracks and helms are taught through host's factories (`poseTrack`, `cameraTrack`, `poseHelm`, `cameraHelm`: host ticks them), tree's classes stay developer-level. The docs build fails when a documented name is missing from the bundle or its parameter names differ from the function's |
| 58 | webgl.tree's script-tag build | #56 (2026-09-14) | UMD `dist/webgl.tree.umd.cjs` (script tag, `require`, AMD), `.cjs` because the package is `"type": "module"` | as p5.tree: ESM plus IIFE `dist/webgl.tree.js`, no `require` path | **p5.tree's pair** (Pierre, 2026-09-14) — ESM `dist/index.js` plus IIFE `dist/webgl.tree.js` (global `webglTree`), the `.cjs` dropped (webgl.tree aa3ce3b) |
| 59 | How the documented host surface reads | #57 (2026-09-14) | host's top-level factories, `host.poseTrack(host, opts)` | the methods of the context `createHost` returns, `canvasHost.poseTrack(opts)` | **pending** — implemented as the context's methods (webgl.tree `docs/host.js`): the same functions without the host argument, the form webgl.tree's examples already write; the top-level factories stay developer-level. Pages: `tree.matrices`, `tree.projection`, `tree.quaternions`, `tree.camera`, `tree.visibility`, `host`, `host.handles`, `host.tracks`; the shared track transport documented once under PoseTrack; a helm's instance-added `bind` / `dispose` named in `poseHelm`'s prose, outside the build check |
| 60 | `ndcZMin`, the viewport and the view bag at the user surface | webgl.tree docs (2026-09-14) | every projection or mapping call takes `ndcZMin`; mappings also take the bag `m` and `vp` | a setter in tree (global state in the core); bind `ndcZMin` only | **the bridge binds them** (Pierre, 2026-09-14) — webgl.tree's `tree` namespace is tree with every `ndcZMin`-taking export wrapped to `WEBGL` and the parameter dropped; `mapLocation`, `mapDirection`, `unproject`, `pixelRatio` and `mat4Viewport` are webgl.tree functions taking `gl` first, reading `viewOf(gl)` and the viewport. `@nakednous/tree` keeps the explicit parameters |
| 61 | Storage for the out-first functions | webgl.tree docs (2026-09-14) | `new Float32Array(16)` written at every call site | allocators in each bridge | **allocators in tree** (Pierre, 2026-09-14) — `mat4()`, `mat3()` identity `Float32Array`s, `vec3()`, `quat()` `[0, 0, 0, 1]`, `planes()` a `Float64Array(24)`; setup-time, each with its golden case. Out stays mandatory; module headers say so |
| 62 | `mat4Bias` on the site | webgl.tree docs (2026-09-14) | readout-only example | drop it from the site | **kept, with a projective example** (Pierre, 2026-09-14) — a camera looks straight down, renders to a target, and its image is laid back on the ground with uv = Bias · P · V · corner; the shadow-mapping hero references it |
| 63 | Which window space `SCREEN` and the pointer name | webgl.tree docs (2026-09-15) | the DOM's: top-left, y down, viewport `[0, h, w, −h]`; host's pointer and labels in CSS pixels, webgl.tree's HUD and `space` functions in drawing-buffer pixels | GL's window coordinates — bottom-left, y up, `[0, 0, w, h]`, device pixels — what the pipeline chapter's `W` and the notation's `pointer` already describe, and `gl_FragCoord` counts | **two spaces, split at the GPU boundary** (Pierre, 2026-09-15) — *canvas space*: the canvas's logical pixels, top-left, y down, tree's signed viewport `[x, y + h, w, −h]`; what the pointer, `unproject`, `pick`, `mapLocation(SCREEN)`, `pixelRatio`, labels and the HUD count, so a DOM number or p5's mouse feeds them as is and a sub-viewport composes through `W`'s origin. *Window space*: GL's, bottom-left, y up, device pixels; what `readPixel`, `image`'s offset, `renderTarget` sizes, `gl.viewport`, `gl_FragCoord` and `uResolution` count. The bridge derives the scale from the drawing buffer over the host's logical width (the canvas's client width without a host). three.js draws the same boundary; its NDC ray input is not adopted since a pixel API composes with sub-viewports and keeps `unproject` as `(W · P · V)⁻¹`. host, p5.tree and tree unchanged. Closes §2.3's "pointer in canvas pixels": canvas space is `W` with the DOM's `E⁻¹` folded in, the same affine shape with h negative. Alternatives weighed: GL's space bound at init and hidden in host (every DOM number silently wrong, HUD sizes in device pixels), pointer as NDC (labels and the HUD still need pixels, p5.tree could not follow) |
| 64 | Crossing from canvas space to window space | §7 #63 (2026-09-15) | the user writes `x · s`, `H − y · s` at every `readPixel` | `readPixel` in canvas pixels (one CSS pixel covers DPR² device pixels; a target's pixels are not the canvas's) | **`fragCoord(gl, out, x, y)`** (Pierre, 2026-09-15) — webgl.tree's space module, out-first into a 2-vector: the `gl_FragCoord` of a canvas pixel on the drawing buffer; the value of a pointer uniform beside `filter`'s `uResolution`, both window space. Targets of another size are the application's layout, no target parameter. Per bridge, since the crossing is per graphics API (webgpu.tree's flips nothing): p5.tree's `fragCoord(x = mouseX, y = mouseY)` returns `[x·pd, (height − y)·pd]` beside `screenSize()`, in its allocating helper style. tree gains the `vec2()` allocator with its golden case, the uniform's storage |
| 65 | The drawing buffer at the display's pixel density | webgl.tree docs (2026-09-16) | every example writes `canvas.width = 400`, `canvas.height = 300` and nothing reads `devicePixelRatio`, so on a DPR 2 display the 400×300 buffer is stretched over 800×600 device pixels and every edge, fills included, is bilinear-soft — what Pierre saw as blur, mistaken for line quality (§7 #54) | (a) each example scales by `devicePixelRatio` and sets a CSS size, four lines of ceremony in 170 examples; (b) the docs runner, which cannot intercept an example's own assignment to `canvas.width`; (c) host's canvas observer writing the buffer, against its rule that the buffer is the application's | **`createCanvas(width, height, opts) → gl`** (Pierre, 2026-09-16) — a bridge verb: creates the element, appends it to `opts.parent` (default `document.body`), sets its CSS size to `width × height`, its buffer to `width · density × height · density` (`opts.density`, default `devicePixelRatio`), gets the WebGL2 context with `opts.attributes`, and returns `gl` — the viewport follows since the buffer is sized before the context. p5's `createCanvas` supplies exactly this silently, and the notation's `width · height` row already reads device pixels; twgl's `resizeCanvasToDisplaySize` is not re-wrapped, since it neither creates, styles nor gets the context, and its target is the laid-out size, not a requested one. Fixed size at creation: zoom changes are the application's (`host.createHost`'s observer reports them). The examples' four setup lines become `const gl = createCanvas(400, 300)`. Then §7 #54 is judged on a sharp buffer; render targets stay non-multisampled unless `samples` asks |
| 66 | The notebook `twgl` shortcode's shape for the ports | the notebook, §4.2 (2026-09-16) | §4.2: the `twgl` shortcode "is already ESM with an importmap and `tree` / `ui` flags; migrated figures take the importmap route, gaining `host` and `webgl.tree` entries; `shared/` and `helpers/` become ES modules". Today three shortcodes: `twgl` maps `twgl` → twgl.js 5.5.4 and pins tree 0.0.27 / ui 0.0.13 behind flags (12 uses); `webgl-tree` maps tree and ui through jsDelivr's unpinned `+esm` (3 uses, the deferred chapter); `webgl` runs a classic script (0 uses). None cache-busts its sketch, each sizes the iframe to the first canvas once | (a) jsDelivr `+esm` for `webgl.tree`: its bare `@nakednous/tree` / `host` / `twgl.js` imports are rewritten to CDN copies, so a sketch's own `@nakednous/tree` is a second module instance; (b) the per-dependency flags kept; (c) a `ground=` closure list for modules as `data/grounds.yaml` keeps for classic scripts; (d) `twgl` kept as an alias of `twgl.js` | **one shortcode, `webgl-tree`, latest versions** (Pierre, 2026-09-16): `{{< webgl-tree sketch="/sketches/x.js" [allow=…] >}}` replaces the three — the package's name with the dot swapped, since Hugo reads a shortcode's name from its filename (`gl-tree` was the other candidate). (1) *Import map, no flags, no pins*: `twgl.js` through jsDelivr's `+esm` (twgl's ESM path carries its major version, and twgl has no dependencies, so the rewrite is harmless), `webgl.tree`, `@nakednous/tree`, `@nakednous/host`, `@nakednous/ui` through unversioned `dist/index.js` URLs, the latest dist-tag; the notebook installs nothing. An unused entry fetches nothing, and since webgl.tree's bundle keeps its bare imports (checked on the CDN), its `tree` / `host` namespaces and the sketch's resolve to one module instance each. Latest rolls at the CDN's cadence and a publish reaches every reader without a notebook commit; a freeze puts a version in the shortcode's head, as `p5.html` did for p5.tree. The 12 existing sketches change `'twgl'` to `'twgl.js'`, the package's name (their nine twgl calls and 25 tree symbols exist in the current builds; Chromium check). (2) *Grounds are modules*: `static/sketches/modules/` holds the ESM grounds and the shortcode maps every file there as `nb/<name>` → `/sketches/modules/<name>.js?v=<content hash>` (Hugo `os.ReadDir`), so a sketch writes `import { palette, onTheme } from 'nb/theme'` and the map carries the iframe cache-bust the p5 shortcode learnt to need; no closure list, since modules import what they need; a hero's private helper stays a relative import. `modules/theme.js` comes first: the palette table, `theme()`, `palette()`, `onTheme(cb)`, the body paint on import; the `createFilterShader` patch stays in the classic `helpers/theme.js`, which the p5 figures keep. (3) *Local override before a publish*: `HUGO_STACK_SRC=/lib hugo server` maps the three stack packages to `static/lib/{tree,host,webgl.tree}.js` (gitignored, the quadrille override's mechanism), content-hashed — what the pilot runs on until tree 0.0.30 / host 0.0.5 / webgl.tree 0.0.4 ship. (4) *Sizing*: the shortcode carries its own copy of the p5 shortcode's full-body measure (every canvas, absolutely positioned controls, ResizeObserver, the 6 s burst then a heartbeat), so a panel under the canvas is not clipped; `p5.html` is untouched, and so are its 150 embeds, the classic grounds and `data/grounds.yaml` — every hero and figurine keeps running, the must. (5) `sketch` gets the content-hash query; `allow` passes through for `camera` / `hid`. `twgl` and `webgl` are deleted once their 12 embeds are on `webgl-tree` (the deferred chapter's three already are; their sketches import only tree and ui) |
| 67 | Where a loaded model's normals, extent and buffer live | the shading ports (2026-09-16) | notebook module `nb/mesh`: `computeNormals(arrays)` (smooth, area-weighted, when the OBJ carries none — bunny, dolphin), `bbox(arrays)` (min, max, center, diag), `meshBuffer(gl, arrays)` (the host / twgl arrays shape renamed to the shaders' aPosition · aNormal · aTexCoord · aTangent) | leave them notebook-side | **pending** (Pierre: "people may need it", as a later step). Proposed split by layer: tree gains `meshNormals(out, positions, indices)` and `meshBounds(out, positions)` (pure numeric, out-first, golden cases); host's `loadModel` computes normals through tree when the file has none (`opts.normals`, default on) and reports the bounds beside the arrays; webgl.tree gains a `buffer(gl, arrays)` verb that renames the arrays shape to the bridge's attribute names (twgl's primitives use the same keys, so one verb serves both). `nb/mesh` then thins to nothing |
| 68 | A panel's bindings in columns | the culling and traveling ports (2026-09-17) | ui lays every binding on its own row; a sketch wanting pairs sets a CSS grid on the panel's body element (`.p5t-body`), the style the p5 helpers applied too | leave it to the sketch | **pending** (Pierre asked that layout stay ui's own machinery). Proposed: a `columns` layout option on `createPanel` (default 1) laying label / control pairs `columns` per row on the body, so no sketch touches the panel's DOM; per-binding `visible` keeps hiding both cells |
