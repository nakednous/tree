...
| 22 | The p5 parity fixture | §2.4, camera-design §7 | one fixture generated from `p5.tree`'s renderer reads | fold the p5 numbers into `golden/camera.json` | **pending** — implemented as a hand-authored `golden/camera.p5.json` (`<module>.<source>.json`): the same shape, replayed by `npm test`, never regenerated by `tools/golden.js`; `cameraPlanes` and `cameraFromMat4` cases, empty until the sketch fills them |
...
Twenty-three rows; sixteen ruled (September 2026), #16 and #18–#23 pending; the table stays as
