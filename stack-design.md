...
| 36 | Inertia on the orbit | host §12 (surfaced by the e13 phone and desktop runs, 2026-09-12) | none — direct manipulation wants exactness | an opt-in `inertia`: a decayed replay of the gesture's last step for orbit, pan and dolly alike, cancelled by the next touch | **a rate that decays in time, orbit only** (Pierre, 2026-09-16; deferred 2026-09-12 with e13's per-frame coast as the departure point) — `inertia` in seconds, the coast's e-folding time `τ`, 0 exact (default). During the drag the orbit stays exact. At release the rate `[dAz, dEl, panX, panY, log dolly]` per second is the sum of the last ~100 ms of steps over their time, so a finger that stops before lifting does not coast; then each frame applies the exact integral `v · τ · (1 − e^(−dt/τ))` and decays `v` by `e^(−dt/τ)`, so the travel is `v₀ · τ` at any frame rate. The wheel joins as impulses on the dolly term (`Δ / τ`, the same travel as the exact case), so zoom stops arriving in bursts. A press, `home()` and `enabled = false` cancel. The clock is the orbit's own, `performance.now()` between updates, clamped. tree holds the numeric piece with its golden case — `coastStep(out, rate, dt, tau)`, the step written and the rate decayed in place, `coastAlive(rate, eps)` — host's orbit the window, the release, the cancel and the application through the camera ops it already uses; p5.tree adopts it unchanged. Not shared: handles position exactly, helms are rates already, tracks are playback — the wider question closes here. Alternatives weighed: e13's largest-step seed and per-frame decay (frame-rate dependent, a stopped finger coasts), the release rate fed to a camera helm (body-relative, drifts off the gaze point), three.js's damping during the drag (the camera lags the finger) |
